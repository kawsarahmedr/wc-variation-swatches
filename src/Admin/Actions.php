...
class Actions {
	...
	public static function add_thing() {
		...
		if ( empty( $data['name'] ) ) {
			wc_variation_swatches()->add_notice( __( 'Please enter a name for the thing.', 'wc-variation-swatches' ), 'error' );
			wp_safe_redirect( $referer );
			exit;
		}
		if ( empty( $data['product_id'] ) ) {
			wc_variation_swatches()->add_notice( __( 'Please select a product for the thing.', 'wc-variation-swatches' ), 'error' );
			wp_safe_redirect( $referer );
			exit;
		}
		$data['status'] = 'publish';
		...
		if ( is_wp_error( $thing ) ) {
			wc_variation_swatches()->add_notice( $thing->get_error_message(), 'error' );
			wp_safe_redirect( $referer );
			exit;
		}
		...
		if ( 'create_order' === $status && $product ) {
			...
			if ( is_wp_error( $order_id ) ) {
				wc_variation_swatches()->add_notice( $order_id->get_error_message(), 'error' );
				wp_safe_redirect( $referer );
				exit;
			}
			$notice = sprintf(
				/* translators: %1$s: order edit link, %2$s: order id */
				__( 'Thing has been created and created an associated order. <a href="%1$s">Edit order #%2$s</a>', 'wc-variation-swatches' ),
			...
			if ( is_wp_error( $order_item_id ) ) {
				wc_variation_swatches()->add_notice( $order_item_id->get_error_message(), 'error' );
				wp_safe_redirect( $referer );
				exit;
			}
			$notice = sprintf(
				/* translators: %1$s: order edit link, %2$s: order id */
				__( 'Thing has been created and updated the associated order. <a href="%1$s">Edit order #%2$s</a>', 'wc-variation-swatches' ),
				esc_url( admin_url( 'post.php?post=' . $order->get_id() . '&action=edit' ) ),
				esc_html( $order->get_id() )
			);
		} else {
			$notice = sprintf(
				...
				__( 'Thing has been created. <a href="%1$s">Edit thing #%2$s</a>', 'wc-variation-swatches' ),
				esc_url( admin_url( 'admin.php?page=wc-variation-swatches&edit_thing=' . $thing->get_id() ) ),
				esc_html( $thing->get_id() )
			);
		}

		wc_variation_swatches()->add_notice( $notice );
		wp_safe_redirect( $referer );
		exit;
	}
