...
$tabs         = apply_filters( 'wc_variation_swatches_' . $page_hook . '_tabs', $tabs );
	...
	<div class="wrap pev-wrap woocommerce">
		<?php if ( ! empty( $tabs ) && count( $tabs ) > 1 ) : ?>
			<nav class="nav-tab-wrapper pev-navbar">
				 ...
				 * @since 1.0.0
				 */
				do_action( 'wc_variation_swatches_' . $page_hook . '_nav_items', $current_tab, $tabs );
				?>
			</nav>
		<?php endif; ?>
		...
		if ( ! empty( $tabs ) && ! empty( $current_tab ) ) {
			 ...
			 * @since 1.0.0
			 */
			do_action( "wc_variation_swatches_{$page_hook}_{$current_tab}_content", $current_tab );
		}
		 ...
		 * @since 1.0.0
		 */
		do_action( "wc_variation_swatches_{$page_hook}_content", $current_tab );
		?>
	</div>
<?php
